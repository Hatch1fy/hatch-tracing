defmodule HatchTracingTest do
  ...
  defmodule Example do
    ...
    def variation_2 do
      start_opts = HatchTracing.build_span_opts(attributes: %{xpto: "xablau"})
    ...
    def variation_3 do
      start_opts = HatchTracing.build_span_opts(kind: :client, attributes: %{xpto: "xablau"})
    ...
    def variation_4 do
      span %{}, span_name() do
        HatchTracing.set_attribute("attrs", %{x: 1, y: 2})
    ...
    def variation_5 do
      span do
        HatchTracing.set_attribute("attrs", {:ok, {%{id: 1}, %{id: 2}}})
        {:ok, {%{id: 1}, %{id: 2}}}
      end
    end
  ...
  describe "span/3" do
    test "creates spans with default name and no attributes" do
      ...
      assert_receive {:span, span(name: "HatchTracingTest.Example.variation_1/0")}
    end
    ...
    test "creates spans with default name and custom attributes" do
      ...
      assert_receive {:span,
                      span(
                        name: "HatchTracingTest.Example.variation_2/0",
    ...
    test "validate result with tuple and struct" do
      ...
      assert_receive {:span,
                      span(
                        name: "HatchTracingTest.Example.variation_5/0",
                        attributes: ^attributes
                      )}
    end
    ...
    test "handles raises in multi spans" do
      ...
      assert_receive {:span,
                      span(
                        name: "HatchTracingTest.Example.variation_7/0",
                        status: {:status, :error, "exception"}
                      )}

      assert_receive {:span,
                      span(
                        name: "HatchTracingTest.Example.variation_6/0",
                        status: {:status, :error, "exception"}
                      )}
    end
  end
